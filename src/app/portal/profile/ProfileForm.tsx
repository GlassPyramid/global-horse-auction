...
export function ProfileForm({ userId, email, fullName, phone, country, role, verified }: Props) {
  ...
  const fields = [
    { field: "fullName", label: t('profile', 'fullName'), icon: User, type: "text", placeholder: t('profile', 'yourFullName'), value: form.fullName, readOnly: false },
    { field: "email", label: t('profile', 'emailAddress'), icon: Mail, type: "email", placeholder: email, value: email, readOnly: true },
    { field: "phone", label: t('profile', 'phone'), icon: Phone, type: "tel", placeholder: "+1 234 567 8900", value: form.phone, readOnly: false },
    { field: "country", label: t('profile', 'country'), icon: Globe, type: "text", placeholder: t('profile', 'yourCountry'), value: form.country, readOnly: false },
  ];
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <h1 className="text-3xl font-bold text-white font-[family-name:var(--font-playfair)]">{t('profile', 'myProfile')}</h1>
        <p className="text-[#7a8fa8] text-sm mt-1 font-[family-name:var(--font-inter)]">{t('profile', 'manageDetails')}</p>
      </div>
          ...
          <div className="flex items-center gap-3 mt-2">
            ...
            {verified && (
              <div className="flex items-center gap-1">
                ...
                <span className="text-xs text-green-400 font-[family-name:var(--font-inter)] font-semibold">{t('profile', 'verified')}</span>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="bg-[#0a1428] rounded-2xl border border-[#c9a84c]/10 p-6 space-y-5">
        <h2 className="font-bold text-white font-[family-name:var(--font-inter)] flex items-center gap-2">
          <User className="w-4 h-4 text-[#c9a84c]" /> {t('profile', 'personalInfo')}
        </h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          {fields.map(({ field, label, icon: Icon, type, placeholder, value, readOnly }) => (
            <div key={field}>
        ...
        <button onClick={handleSave} disabled={saveState === "loading" || saveState === "success"}
          ...
          {saveState === "success" ? <><CheckCircle className="w-4 h-4" /> {t('profile', 'savedConfirm')}</> : saveState === "loading" ? t('profile', 'saving') : <><Save className="w-4 h-4" /> {t('profile', 'saveChanges')}</>}
        </button>
      </div>

      <div className="bg-[#0a1428] rounded-2xl border border-[#c9a84c]/10 p-6 space-y-5">
        <h2 className="font-bold text-white font-[family-name:var(--font-inter)] flex items-center gap-2">
          <Lock className="w-4 h-4 text-[#c9a84c]" /> {t('profile', 'changePassword')}
        </h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label className="text-xs text-[#4a5a70] uppercase tracking-widest font-[family-name:var(--font-inter)] mb-2 block">{t('profile', 'newPassword')}</label>
          ...
          <div>
            <label className="text-xs text-[#4a5a70] uppercase tracking-widest font-[family-name:var(--font-inter)] mb-2 block">{t('profile', 'confirmPassword')}</label>
        ...
        <button onClick={handlePasswordChange} disabled={pwState === "loading" || pwState === "success"}
          ...
          {pwState === "success" ? <><CheckCircle className="w-4 h-4" /> {t('profile', 'passwordUpdated')}</> : pwState === "loading" ? t('profile', 'updating') : t('profile', 'updatePassword')}
        </button>
      </div>
    </div>
  );
