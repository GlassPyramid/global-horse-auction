...
import { mockHorses, mockAuctions, mockUser } from "@/lib/mock-data";
import { formatCurrency, formatDateTime } from "@/lib/utils";

const allBids = mockHorses.flatMap((h) =>
  h.bids.map((b) => ({ ...b, horse: h }))
).sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());

export default function AdminDashboard() {
  const totalRevenue = allBids.reduce((sum, b) => sum + b.amount * 0.1, 0); // 10% buyer's premium estimate
  const liveAuctions = mockAuctions.filter((a) => a.status === "LIVE");

  const kpis = [
    { icon: Gavel, label: "Total Bids", value: allBids.length.toString(), sub: "Across all auctions", color: "text-[#c9a84c]" },
    { icon: TrendingUp, label: "Est. Revenue", value: formatCurrency(totalRevenue), sub: "10% buyer's premium", color: "text-green-400" },
    { icon: Footprints, label: "Active Lots", value: mockHorses.length.toString(), sub: `${liveAuctions.length} auction live`, color: "text-blue-400" },
    { icon: Users, label: "Registered Users", value: "12,847", sub: "Verified bidders", color: "text-purple-400" },
  ];
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div className="text-xs text-[#7a8fa8] font-[family-name:var(--font-inter)] flex items-center gap-1 mt-0.5">
                    ...
                    {auction.horses.length} lots · Closes {new Date(auction.endDate).toLocaleDateString()}
                  </div>
                </div>
                <Link href={`/admin/auctions/${auction.id}`} className="text-[#c9a84c] hover:text-[#e2c97e]">
        ...
        <div className="bg-[#0a1428] rounded-2xl border border-[#c9a84c]/10 overflow-hidden">
          ...
          <div className="divide-y divide-[#c9a84c]/8">
            {allBids.slice(0, 6).map((bid) => (
              <div key={bid.id} className="px-6 py-3 flex items-center justify-between gap-4">
                <div className="min-w-0">
                  <div className="text-xs font-semibold text-white font-[family-name:var(--font-inter)] truncate">{bid.horse.name}</div>
              ...
              {mockHorses.map((horse) => (
                <tr key={horse.id} className="hover:bg-[#c9a84c]/2 transition-colors">
                  <td className="px-6 py-4">
                    <Link href={`/admin/horses/${horse.id}`} className="font-semibold text-white hover:text-[#c9a84c] transition-colors font-[family-name:var(--font-inter)]">
                      {horse.name}
                    </Link>
                  </td>
                  ...
                  <td className="px-6 py-4 font-bold text-[#c9a84c] font-[family-name:var(--font-inter)]">
                    {formatCurrency(horse.currentPrice)}
                  </td>
                  <td className="px-6 py-4 text-[#7a8fa8] font-[family-name:var(--font-inter)]">{horse.bids.length}</td>
                  <td className="px-6 py-4">
