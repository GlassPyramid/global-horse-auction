...
export function EditHorseForm({ horse, auctions }: { horse: Horse; auctions: Auction[] }) {
  const id = horse.id as string;
  const router = useRouter();
  const [state, setState] = useState<"idle" | "loading" | "success" | "error">("idle");
  const [error, setError] = useState("");
  const [images, setImages] = useState<string[]>((horse.images as string[]) ?? []);
  const [form, setForm] = useState({
    name: (horse.name as string) ?? "",
    breed: (horse.breed as string) ?? "",
    age: String(horse.age ?? ""),
    gender: (horse.gender as string) ?? "MARE",
    color: (horse.color as string) ?? "",
    heightCm: String(horse.height_cm ?? ""),
    country: (horse.country as string) ?? "",
    sire: (horse.sire as string) ?? "",
    dam: (horse.dam as string) ?? "",
    discipline: (horse.discipline as string) ?? "Dressage",
    category: (horse.category as string) ?? "COMPETITION_READY",
    description: (horse.description as string) ?? "",
    startingPrice: String(horse.starting_price ?? ""),
    currency: (horse.currency as string) ?? "EUR",
    lotNumber: String(horse.lot_number ?? ""),
    vetChecked: (horse.vet_checked as boolean) ?? false,
    featured: (horse.featured as boolean) ?? false,
    videoUrl: (horse.video_url as string) ?? "",
    auctionId: (horse.auction_id as string) ?? "",
  });
        ...
        <div className="lg:col-span-2 space-y-6">
          <FormSection title="Basic Information">
            <div className="grid grid-cols-2 gap-4">
              ...
              <Field label="Country" value={form.country} onChange={set("country")} placeholder="e.g. Netherlands" required />
              <SelectField label="Discipline" value={form.discipline} onChange={set("discipline")}>
                {disciplines.map((d) => <option key={d} value={d}>{d}</option>)}
              </SelectField>
              <Field label="Sire" value={form.sire} onChange={set("sire")} placeholder="Optional" />
              <Field label="Dam" value={form.dam} onChange={set("dam")} placeholder="Optional" />
            </div>
          </FormSection>

          <FormSection title="Description">
            <div>
              <label className="text-[10px] text-[#4a5a70] uppercase tracking-widest font-[family-name:var(--font-inter)] mb-2 block">Full Description</label>
              <textarea value={form.description} onChange={set("description")} rows={8}
                className="w-full bg-[#060c1d] border border-[#c9a84c]/20 rounded-xl px-4 py-3 text-sm text-white font-[family-name:var(--font-inter)] focus:outline-none focus:border-[#c9a84c] transition-colors resize-none" />
            </div>
            <Field label="Video URL" value={form.videoUrl} onChange={set("videoUrl")} placeholder="https://youtube.com/..." />
          </FormSection>
