    ...
    <div className="group relative bg-[#0a1428] border border-[#c9a84c]/10 hover:border-[#c9a84c]/35 rounded-xl overflow-hidden transition-all duration-500 hover:shadow-2xl hover:shadow-[#c9a84c]/5 hover:-translate-y-1">
      ...
      <div className={cn("relative overflow-hidden", compact ? "h-48" : "h-64")}>
        ...
        <div className="absolute top-3 left-3 flex gap-2">
          <span
            className={cn(
              "text-[10px] font-bold px-2.5 py-1 rounded-full border tracking-wider uppercase font-[family-name:var(--font-inter)]",
              categoryClass(horse.category)
            )}
          >
      ...
      <div className="p-5">
        ...
        <div className="grid grid-cols-3 gap-2 mb-4">
          {[
            ...
            { label: "Height", value: `${(horse.heightCm / 10.16).toFixed(1)} hh` },
            { label: "Discipline", value: horse.discipline },
          ].map((spec) => (
            <div
              key={spec.label}
              className="bg-[#060c1d]/60 rounded-lg px-3 py-2 text-center border border-[#c9a84c]/8"
            >
